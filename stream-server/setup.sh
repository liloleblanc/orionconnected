...
STREAM_URL="${STREAM_URL:-$(_saved STREAM_URL)}"
STREAM_URL="${STREAM_URL:-https://fids.orionconnected.com/rotate.html?ap=YQM&mode=live&stream=1&langs=en,fr&rotate=fids,gids,bids,gids&dwell=45}"

# ── Presets ───────────────────────────────────────────────────────────────
# Stream 2 — MIAMI, English + Spanish (was Tampa). Use with:
#   STREAM_URL="$MIAMI_URL" MUSIC_URL="$ORION_RADIO" bash setup.sh
MIAMI_URL="https://fids.orionconnected.com/rotate?ap=MIA&mode=live&stream=2&langs=en,es&rotate=gids,fids,gids,bids&dwell=60}"
# The station Nick uses for the stream's background audio.
ORION_RADIO="http://prem1.di.fm:80/nudisco_hi?69e5e3fba85f75f83ad9886d"
