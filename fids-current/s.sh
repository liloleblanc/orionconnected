#!/usr/bin/env bash
#
# NOTE: served mirror of stream-server/setup.sh, published here so a server
# can fetch it with a short, typeable URL:  wget fids.orionconnected.com/s.sh
# The Hetzner web console drops characters on long lines, which makes the
# raw.githubusercontent.com URL genuinely hard to enter by hand. Keep this in
# step with stream-server/setup.sh — that file remains the source of truth.
#
