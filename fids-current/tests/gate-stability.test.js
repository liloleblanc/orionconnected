...
const css = fs.readFileSync(path.join(root, 'css', 'fids-layout-fixes.css'), 'utf8');
...
test('all display entry points load the date-context helper before core', () => {
  for (const file of ['fids.html', 'gids.html', 'bids.html']) {
    ...
    const helper = html.indexOf('js/gate-date-context.js?v=23147');
    const main = html.indexOf('js/fids-core.js?v=23147');
