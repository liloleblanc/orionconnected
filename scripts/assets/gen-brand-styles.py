#!/usr/bin/env python3
"""
Generate the PER-BRAND IDENTITY CSS section of css/hotel-ads.css.
...
Edit the TABLE below to tune any brand, then re-run:  python3 scripts/assets/gen-brand-styles.py --write
...
header="""/* ════════════════════════════════════════════════════════════════════════
   PER-BRAND IDENTITY — AUTO-GENERATED by scripts/assets/gen-brand-styles.py
   Same slide structure for every hotel; this swaps palette + type per brand,
...
out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "brand-styles.generated.css")
with open(out_path, "w") as f:
    f.write(out)
print("brands:", len(TABLE), "| wrote", out_path, len(out), "bytes")
