import AppKit
// Multiply alpha by a smooth ramp from 1 at row r0 to 0 at row r1 (top-down rows), zero below r1. args: in out r0 r1
...
for row in 0..<H {  // CG rows are bottom-up; convert
  let y = H - 1 - row
  var k = 1.0
  if y >= r1 { k = 0 } else if y > r0 { let t = Double(y - r0) / Double(r1 - r0); k = 1 - t*t*(3 - 2*t) }
